local _key = "100100"
local _RAM = {100, 93, 20, 10, 60, 40, 22, 44, 66}
local max
...
m.__index = newcclosure(function(t, k)
    if k == "hp" and t:IsA("Humanoid") then
        return 20
    end
    return oldIndex(t, k)
end)
...
local coreGui = pl:WaitForChild("PlayerGui")
...
b3.MouseButton1Click:Connect(function()
    ...
    pcall(function()
        getCh().Humanoid.WalkSpeed = t.s and 100 or 16 and 80
    end)
end)
...
m.__index = newcclosure(function(t, k)
    if k == "WalkSpeed" and t:IsA("Humanoid") then
        return 16
    end
    return oldIndex(t, k)
end)
...
local function admin(coin)
if coin > 99 then
      end 
end
if get_hidden_gui or protect_gui then
    if protect_gui then
        protect_gui(sg)
        sg.Parent = coreGui
